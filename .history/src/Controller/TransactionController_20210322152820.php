...
{ 
    ...
    public function depotTransaction(Request $request,SerializerInterface $serializer, EntityManagerInterface $entityManager,
       ...
       /** @Route
       *  ( path="/api/users/comptes/transaction/{codeTransaction}", methods={"POST"},
       *  
       * )
       */
        public function retraitTransaction(TransactionsRepository $transRepo, Request $request) {
            //dd('test');
           $transaction = $transRepo->findByCodeTransaction(['codeTransaction']); 
           dd($transaction);
