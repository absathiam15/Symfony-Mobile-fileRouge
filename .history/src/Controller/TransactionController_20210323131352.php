...
{ 
    ...
    public function depotTransaction(Request $request,SerializerInterface $serializer, EntityManagerInterface $entityManager,
       ...
       * )
       */
        public function retraitTransaction(TransactionsRepository $transRepo, $codeTransaction, $clients, TransactionServices $transactionService, EntityManagerInterface $entityManager, Request $request) {
            //dd('test');
            $trans = $transRepo->findBy(['codeTransaction'=>$codeTransaction]); 
            $trans = $transRepo->findBy(['clients'=>$clients]);
           // dd($trans);
            ...
            $clientRetrait = $trans->getClients();
            $trans->setClients($clientRetrait);

           // dd($trans);

            $dateRetrait = new \DateTime();
            $trans->setDateRetrait($dateRetrait);
            dd($trans);
           ...
           $entityManager->persist($dateRetrait);
          //  dd($transaction);
